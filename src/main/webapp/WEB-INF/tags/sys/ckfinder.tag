...
<a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'选择'}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">清除</a><span class="promptPic"></span><ol id="${input}Preview"></ol><c:if test="${!readonly}"></c:if>
<script type="text/javascript">
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		// 过滤选择的文件中重复的文件
		for(var i in files){
		    if(arr.indexOf(files[i]) == -1){
		        arr.push(files[i]);
            }
        }
        // 解析已经在页面上显示的文件
        var originlVal = $("#${input}").val();
        if(originlVal.length>1){
            var originArr = originlVal.split("|");
            for(var i in originArr){
                var allUrl = originArr[i];
                if(allUrl == "") continue;
                originArr[i] = allUrl.substr(allUrl.lastIndexOf("/")+1);
                // 过滤已经显示在页面上的文件
                for(var k in arr){
                    var encodeOriginalFile = encodeURI(arr[k]['name']);
                    if(originArr[i] == encodeOriginalFile){
                        arr.splice(k,1);
                    }
                }
            }
