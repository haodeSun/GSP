...
<body>
	...
	<ul class="breadcrumb">
		...
		<li class="active">首营产品审批详情</li>
	</ul>

	<div id="topTitle">首营产品审批详情</div>
	...
	<form:form id="inputForm" modelAttribute="t01ComplProd" action="${ctx}/gsp/t01complprod/t01ComplProd/save" method="post" class="form-horizontal">
		...
		<div id="pagingDiv" class="table-scrollable">
			...
			${t01ComplProd.apprStat=='1'?'noApproval':''}
			${t01ComplProd.apprStat=='2'?'passApproval':''}
			${t01ComplProd.apprStat=='3'?'rejectApproval':''}
			"></div>
	...
	<form:form id="auditForm" modelAttribute="t01ComplProd" action="${ctx}/gsp/t01complprod/t01ComplProd/apprLogic" method="post"  class="form-horizontal approvalForm">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
        ...
        <div id="footBtnDiv" class="form-actions">
            <div class="control-group">
                <label class="control-label">
                    ...
                    审批意见：</label>
				<div class="controls">
					<form:textarea path="act.comment" maxlength="250" class="input-xlarge required"/>
				</div>
                <span class="promptPic"></span>
            </div>

            <input class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
            <input class="btn" type="submit" value="退回发起人" onclick="$('#flag').val('0')"/>
            <input class="btn btn-primary" type="submit" value="逐级退回" onclick="$('#flag').val('2')"/>
            <input class="btn btn-primary" type="submit" value="同 意" onclick="$('#flag').val('1')"/>

        </div>


		<act:histoicFlow procInsId="${t01ComplProd.procInsId}" />
