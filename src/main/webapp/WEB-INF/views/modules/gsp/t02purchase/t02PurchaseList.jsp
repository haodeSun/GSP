...
<body>
	...
	<!-- 每页的title（文字换为变量） -->
	<div id="topTitle">采购信息删除</div>
	<!--20161113-->
	<form:form id="searchForm" modelAttribute="t02Purchase" action="${ctx}/gsp/t02purchase/t02Purchase/" method="post" class="breadcrumb form-search">
	...
	<shiro:hasPermission name="gsp:t02purchase:t02Purchase:edit">
		<a href="${ctx}/gsp/t02purchase/t02Purchase/delete?id=${t02Purchase.id}" onclick="return confirmx('确认要删除该采购信息吗？', this.href)"><span class="newBtn btn btn-primary">删除</span></a>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>订单总价</th>
				<th>订单类型</th>
				<shiro:hasPermission name="gsp:t02purchase:t02Purchase:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="t02Purchase">
			<tr>
				<td><input type="checkbox" onchange="checkAll()"/></td>
				<td><a href="${ctx}/gsp/t02purchase/t02Purchase/form?id=${t02Purchase.id}">
					${t02Purchase.procInsId}
				</a></td>
				<td>
					${t02Purchase.purcNumb}
				</td>
				<td>
					<fmt:formatDate value="${t02Purchase.purcDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					${t02Purchase.suggVendCont}
				</td>
				<td>
					${t02Purchase.mateId}
				</td>
				<td>
					${t02Purchase.reviewStatus}
				</td>
				<td>
					${t02Purchase.totalOrderPrice}
				</td>
				<td>
					${t02Purchase.orderType}
				</td>
			</tr>
		</c:forEach>
		</tbody>
