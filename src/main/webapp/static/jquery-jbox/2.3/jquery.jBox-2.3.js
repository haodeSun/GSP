...
(function (a) {
	a.jBox = function (b, c) {
		...
		var i = typeof c.width == 'number' ? (c.width - 0x32)*2 + 'px' : "90%";
		var j = [];
		...
		a.each(b, function (N, O) {
			...
			/*改auto -- c.width + 'px'*/r.push('<div style="min-width:50px;width:' + (typeof c.width == 'number' ? c.width + 'px' : 'auto') + '; height:' + Q + ';">' + R + '<div id="jbox-content" class="jbox-content" style="height:' + Q + ';overflow:hidden;overflow-y:auto;">' + O.content + '</div></div>');
			r.push('<div class="jbox-button-panel" style="height:40px;padding:5px 0 5px 0;text-align: right;' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
	...
	a.jBox.messager = function (b, c, d, e) {
		...
		i.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:' + (g + e.border) + 'px;left:' + (0xa + e.border) + 'px; width:0px; height:0px;"></span>');
		i.push(e.content);
		i.push('</div>');
		e.content = i.join('');
		a.jBox(e.content, e)
