...
import com.thinkgem.jeesite.modules.sys.dao.SysUserConfigDao;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public List<Menu> findAllCertScop() {
		List<Menu> result = new ArrayList<>();
		//风险分类
		List<Dict> risk = DictUtils.getDictList("t01_riskClass");
		//技术分类-名称
		List<Dict> tech = DictUtils.getDictList("t01_tech_cate_cd");
		Menu parent = new Menu();
		parent.setId("0");
		for (Dict itemRisk : risk) {
			Menu menuRisk = new Menu();
			menuRisk.setParent(parent);
			menuRisk.setId(itemRisk.getValue());
			menuRisk.setName(itemRisk.getLabel());
			result.add(menuRisk);
			for (Dict itemTech : tech) {
				Menu menuTech = new Menu();
				menuTech.setParent(menuRisk);
				menuTech.setId(itemRisk.getValue() + "-" + itemTech.getValue());
				menuTech.setName(itemRisk.getLabel() + "-" + itemTech.getLabel());
				result.add(menuTech);
			}
		}
		return result;
	...
	public List<Menu> findMenuBySearch(String menuName){
		Menu menu = new Menu();
		menu.setUserId(UserUtils.getUser().getId());
		menu.setName(menuName);
		return menuDao.findBySearch(menu);
	}
...
//		System.out.println(sb.toString());
	...
	public void setSysUserConfig(SysUserConfig sysUserConfig) {
		if (StringUtils.isBlank(sysUserConfig.getConfigParam())
				|| StringUtils.isBlank(sysUserConfig.getConfigType())) {
			throw new IllegalArgumentException("设置用户配置参数错误");
		}
		SysUserConfig search = new SysUserConfig();
		search.setUser(UserUtils.getUser());
		search.setConfigType(sysUserConfig.getConfigType());
		search.setConfigParam(sysUserConfig.getConfigParam());
		List<SysUserConfig> sysUserConfigList = sysUserConfigDao.findList(search);
		if (sysUserConfigList != null && !sysUserConfigList.isEmpty()) {
			sysUserConfig.setId(sysUserConfigList.get(0).getId());
			sysUserConfig.setUser(UserUtils.getUser());
			sysUserConfig.preUpdate();
			sysUserConfigDao.update(sysUserConfig);
		} else {
			sysUserConfig.preInsert();
			sysUserConfig.setUser(UserUtils.getUser());
			sysUserConfigDao.insert(sysUserConfig);
		}
	}


	public SysUserConfig getSysUserConfig(SysUserConfig sysUserConfig) {
		if (StringUtils.isBlank(sysUserConfig.getConfigParam())
				|| StringUtils.isBlank(sysUserConfig.getConfigType())) {
			throw new IllegalArgumentException("设置用户配置参数错误");
		}
		SysUserConfig result = new SysUserConfig();
		SysUserConfig search = new SysUserConfig();
		search.setUser(UserUtils.getUser());
		search.setConfigType(sysUserConfig.getConfigType());
		search.setConfigParam(sysUserConfig.getConfigParam());
		List<SysUserConfig> sysUserConfigList = sysUserConfigDao.findList(search);
		if (sysUserConfigList != null && !sysUserConfigList.isEmpty()) {
			result = sysUserConfigList.get(0);
		}
		return result;
